...
#include "ui_zqmccontroldialog.h"
...
std::list<std::string> ZQMCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZQMCControlDialog::listMints;

ZQMCControlDialog::ZQMCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZQMCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZQMCControlDialog::~ZQMCControlDialog()
{
    delete ui;
}

void ZQMCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQMCControlDialog::updateList()
{
...
void ZQMCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQMCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZQMC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZQMCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZQMCControlDialog::GetSelectedMints()
{
...
void ZQMCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
