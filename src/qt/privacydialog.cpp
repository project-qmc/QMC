                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zQMC ought to be enough for anybody." - Bill Gates, 2017
    ui->zQMCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzQMCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzQMC->setEnabled(false);
        ui->pushButtonMintzQMC->setToolTip(tr("zQMC is currently disabled due to maintenance."));

        ui->pushButtonSpendzQMC->setEnabled(false);
        ui->pushButtonSpendzQMC->setToolTip(tr("zQMC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zQMCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzQMC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQMC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zQMC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zQMC in ") +
...
void PrivacyDialog::on_pushButtonSpendzQMC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQMC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zQMC
        sendzQMC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zQMC
    sendzQMC();
}

void PrivacyDialog::on_pushButtonZQMCControl_clicked()
{
    ZQMCControlDialog* zQMCControl = new ZQMCControlDialog(this);
    zQMCControl->setModel(walletModel);
    zQMCControl->exec();
}

void PrivacyDialog::setZQMCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzQMCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzQMC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid QMC Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zQMCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zQMC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " QMC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zQMC</b>";
    ...
    // use mints from zQMC selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZQMCControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZQMCControlDialog::GetSelectedMints();
    }
    ...
    // Clear zqmc selector in case it was used
    ZQMCControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zQMC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " QMC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zQMC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zQMC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zQMC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zQMC "));
    ui->labelzQMCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzQMCSyncStatus->setVisible(fShow);
}
