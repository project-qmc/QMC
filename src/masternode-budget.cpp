...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    //Approx 10% of all blocks in a month
    CAmount nSubsidy = 0;
...
bool CBudgetProposal::IsValid(std::string& strError, bool fCheckCollateral)
{
    if (GetNays() - GetYeas() > mnodeman.CountEnabled(ActiveProtocol()) / 10) {
        strError = "Proposal " + strProposalName + ": Marked for active removal";
        return false;
    }
    ...
    if (GetTotalPaymentCount() != 1) {
        strError = "Proposal " + strProposalName + ": Invalid - Only 1 payment allowed per link!";
        return false;
    }

    if (nAmount > 5 * COIN) {
        strError = "Proposal " + strProposalName + ": Invalid nAmount - Maximum 5 Coins";
        return false;
    }
