...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a QMC initiative\n"
            "  vote-alias         - Vote on a QMC initiative\n"
            "  vote               - Vote on a QMC initiative/budget\n"
...
Value preparebudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"qmc-address\" monthy-payment\n"
            ...
            "5. \"qmc-address\":   (string, required) QMC address to send payments to\n"
            "6. monthly-payment:  (numeric, required) Monthly payment amount\n"

            "\nResult:\n"
            "\"xxxx\"       (string) proposal fee hash (if successful) or error message (if failed)\n"
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"https://forum.qmc.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"https://forum.qmc.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));

    if (pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");

    std::string strProposalName = SanitizeString(params[0].get_str());
    if (strProposalName.size() > 200)
        throw runtime_error("Invalid proposal name, limit of 200 characters.");

    std::string strURL = SanitizeString(params[1].get_str());
    if (strURL.size() > 300)
        throw runtime_error("Invalid url, limit of 300 characters.");
...
Value submitbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"qmc-address\" monthy-payment \"fee-tx\"\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"https://forum.qmc.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"https://forum.qmc.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
    ...
    std::string strURL = SanitizeString(params[1].get_str());
    if (strURL.size() > 300)
        throw runtime_error("Invalid URI, limit of 300 characters. Drop some trackers or submit only hash!");
...
Value getbudgetprojection(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) QMC address of payment\n"
